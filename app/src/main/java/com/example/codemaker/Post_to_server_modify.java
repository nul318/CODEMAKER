...
public class Post_to_server_modify extends Activity {
	String key = ""; //파일 path
	String pw = ""; //파일 pw
	...
	private void sendJsonDataToServer(String title, String fileurl , String author, String contents, String pw) throws UnsupportedEncodingException {
		...
		String URL = "http://Server IP/phpcodemaker/modify_proc.php";
		// 1
		...
		nameValuePairs.add(new BasicNameValuePair("fileurl", URLEncoder.encode(fileurl,"UTF-8")));
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final EditText author = (EditText) findViewById(R.id.author_edit);
		...
		Intent intent = this.getIntent(); // 데이터 받아옴
		String pretitle = intent.getStringExtra("title");
		String preauthor = intent.getStringExtra("author");
		String precontents = intent.getStringExtra("contents");
		String prefileurl = intent.getStringExtra("fileurl");
		
		title.setText(pretitle);
		author.setText(preauthor);
		contents.setText(precontents);
		fileurl.setText(prefileurl);
		...
		View.OnClickListener listener = new View.OnClickListener() { // 꿀 형식 모든
																		// 뷰에
																		// 리스너
																		// 장착 가능
...
//				final FTPClient mFTP = new FTPClient();
				...
				new AlertDialog.Builder(Post_to_server_modify.this).setTitle("패스워드를 입력하세요.").setView(addLayout)
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if (!fileurl.getText().toString().equals("FileName")) {
							...
							time = now.get(now.YEAR) % 100 + "" + (now.get(now.MONTH) + 1) + ""
									...
									+ now.get(now.MILLISECOND) + ".mp3"; // 시간으로
																			// 서버에
																			// 저장
...
//										mFTP.disconnect();
								...
								handler.post(new Runnable() {
									...
									public void run() {
										...
										Intent intent = new Intent(Post_to_server_modify.this, Board.class);
	...
	// set dialog message
		...
		View.OnClickListener listener2 = new View.OnClickListener() { // 꿀 형식 모든
			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
